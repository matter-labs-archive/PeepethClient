...
class Web3swiftService {
    ...
    static var peepethContract: web3.web3contract? {
        guard let contract = Web3swiftService.web3instance.contract(peepEthABI, at: ethContractAddress, abiVersion: 2) else {
            return nil
        }
        return contract
    }
    ...
    func prepareCreateAccountTransaction(username: String, userDataHash: String, gasLimit: BigUInt = 27500, completion: @escaping (Result<TransactionIntermediate>) -> Void) {
        ...
        DispatchQueue.global().async {
            ...
            guard let contract = web3.contract(peepEthABI, at: ethContractAddress, abiVersion: 2) else {
                return
            }
            ...
            guard let transaction = contract.method("createAccount", parameters: [username, userDataHash] as [AnyObject], options: options) else {
                return
            }
    ...
    func preparePostPeepTransaction(peepDataHash: String, gasLimit: BigUInt = 27500, completion: @escaping (Result<TransactionIntermediate>) -> Void) {
        DispatchQueue.global().async {
            ...
            options.gasPrice = gasPrice
            options.gasLimit = gasLimit
            guard let transaction = contract.method("post", parameters: [peepDataHash] as [AnyObject], options: options) else {
                return
            }
            guard case .success(let estimate) = transaction.estimateGas(options: options) else {
                return
    ...
    func prepareSharePeepTransaction(peepDataHash: String, gasLimit: BigUInt = 27500, completion: @escaping (Result<TransactionIntermediate>) -> Void) {
        DispatchQueue.global().async {
            ...
            options.gasPrice = gasPrice
            options.gasLimit = gasLimit
            guard let transaction = contract.method("share", parameters: [peepDataHash] as [AnyObject], options: options) else {
                return
            }
            guard case .success(let estimate) = transaction.estimateGas(options: options) else {
                return
    ...
    func prepareTransactionToContract(dataHash: String, contractAbi: String, contractAddress: String, method: String, gasLimit: BigUInt = 27500, completion: @escaping (Result<TransactionIntermediate>) -> Void) {
        DispatchQueue.global().async {
            let wallet = Web3swiftService.keyservice.selectedWallet()
            guard let address = wallet?.address else {
                return
            ...
            options.gasPrice = gasPrice
            options.gasLimit = gasLimit
            guard let transaction = contract.method(method, parameters: [dataHash] as [AnyObject], options: options) else {
                return
            }
            guard case .success(let estimate) = transaction.estimateGas(options: options) else {
                return
    ...
    func prepareReplyPeepTransaction(peepDataHash: String, gasLimit: BigUInt = 27500, completion: @escaping (Result<TransactionIntermediate>) -> Void) {
        DispatchQueue.global().async {
            ...
            options.gasPrice = gasPrice
            options.gasLimit = gasLimit
            guard let transaction = contract.method("reply", parameters: [peepDataHash] as [AnyObject], options: options) else {
                return
            }
            guard case .success(let estimate) = transaction.estimateGas(options: options) else {
                return
